...
CREATE TABLE IF NOT EXISTS `usa_tutor`.`sale_order` (
  ...
  `order_status` TINYINT(2) UNSIGNED NOT NULL COMMENT '订单状态 1:待付款 2:已付款 3:已取消 4:履约中 6:履约完成 7:申请退款 8:已退款',
  `total_original_amount` DECIMAL(10,2) NOT NULL COMMENT '原价',
  `total_due_amount` DECIMAL(10,2) NOT NULL COMMENT '应付',
  `total_discount_amount` DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '折扣总金额',
  `total_paid_amount` DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '实付',
  `paid_time` DATETIME NULL COMMENT '支付时间',
  `total_refundable_amount` DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '总可退款金额',
  `total_refunded_amount` DECIMAL(10,2) NOT NULL DEFAULT 0 COMMENT '总已退款金额',
  `refund_status` TINYINT(2) NOT NULL DEFAULT 0 COMMENT '退款状态 0不可退款 1可退款 2 退款中 3 退款成功 4 退款失败',
...
CREATE TABLE IF NOT EXISTS `usa_tutor`.`sale_order_course` (
  ...
  `couse_tutor_id` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `usa_tutor`.`sale_order_payment` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `payment_type` TINYINT(4) UNSIGNED NOT NULL DEFAULT 1 COMMENT '支付类型 1 支付 2 退款',
  `payment_method_id` TINYINT(4) UNSIGNED NOT NULL COMMENT '支付方式 1 微信 2 支付宝',
